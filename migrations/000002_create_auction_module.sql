...
-- Tournament-scoped event: name + rules JSON + optional parent row (e.g. subgroup / round).
CREATE TABLE IF NOT EXISTS tournament_events (
    id VARCHAR(36) PRIMARY KEY,
    tournament_id VARCHAR(36) NOT NULL REFERENCES tournaments(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    attrs JSONB NOT NULL DEFAULT '{}'::jsonb,
    parent_event_id VARCHAR(36) REFERENCES tournament_events(id) ON DELETE SET NULL,
    created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
);

CREATE INDEX IF NOT EXISTS idx_tournament_events_tournament_id ON tournament_events(tournament_id);
CREATE INDEX IF NOT EXISTS idx_tournament_events_parent_event_id ON tournament_events(parent_event_id);

CREATE TABLE IF NOT EXISTS tournament_team_registrations (
    id VARCHAR(36) PRIMARY KEY,
    tournament_id VARCHAR(36) NOT NULL REFERENCES tournaments(id) ON DELETE CASCADE,
    tournament_event_id VARCHAR(36) NOT NULL REFERENCES tournament_events(id) ON DELETE CASCADE,
    team_name VARCHAR(255) NOT NULL,
    team_logo_url VARCHAR(2048) NOT NULL DEFAULT '',
    created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
);

CREATE INDEX IF NOT EXISTS idx_team_regs_tournament_tevent ON tournament_team_registrations(tournament_id, tournament_event_id);
...
CREATE INDEX IF NOT EXISTS idx_player_regs_tournament_tevent ON tournament_player_registrations(tournament_id, tournament_event_id);
...
CREATE INDEX IF NOT EXISTS idx_auctions_tournament_tevent ON auctions(tournament_id, tournament_event_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_wallets_tournament_tevent_team ON wallets(tournament_id, tournament_event_id, team_id);
